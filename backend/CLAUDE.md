...
- SVG set symbols cached to `static/images/` as `{set_id}.svg`, with `static/images/versions.json` mapping each id to `{version, etag, last_modified}`. Lookups pass the current version; a mismatch revalidates with a conditional request (`If-None-Match` / `If-Modified-Since`) and only downloads on a 200. This matters because Scryfall's icon version query is a global cache-buster that rolls for *every* set at once (~28 days), so without revalidation one roll would re-download ~1000 unchanged icons. Their ETag is the MD5 of the content, so a cached file with no recorded ETag is revalidated by hashing its bytes. Keeping filenames stable lets the committed cache serve as a warm cache across restarts
